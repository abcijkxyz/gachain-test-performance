...
PERF_HOST = 'http://127.0.0.1'  # 日志接收的服务地址
REPORT_LOCUST_LOGS_URL = '/logs'  # 日志API url
LOGGING_SECURE = True  # logging 发送请求方式 False ->http; True->https
...
class LocustLogger():
    ...
    def get_requests_log(self, method, path, requests, response):
        ...
        addrfile = open("locust  test log.txt", 'a', encoding='utf-8')
        addrfile.writelines(json.dumps(msg) + '\n')

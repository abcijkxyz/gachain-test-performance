...
class WebsiteTasks(TaskSet):
    ...
    def history(self):
        ...
        res = self.client.post('/nodelistWhere/history??limit=10&offset=0&order=created_at desc&where={ "ecosystem":"1","$or":[{"sender_id":"' + keyid + '"},{"recipient_id":"' + keyid + '"}],"recipient_id":"' + keyid + '","comment":{"$neq":"New Mine Pledge"}}',
                               headers={'Authorization': login_token})
        print('actually res.status_code is {}'.format(res.status_code))
...
if __name__ == "__main__":
    # os.system('locust -f gac_locust_run_slave8.py --slave')
    os.system('locust -f gac_locust_run_slave8.py --no-web -c 1 -r 1')
